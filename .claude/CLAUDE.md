...
- Charts use `layerchart` primitives wrapped by the shadcn-svelte `Chart.Container` / `Chart.Tooltip` in `$lib/components/ui/chart`. The `ChartConfig` map's `--color-{key}` CSS vars are auto-injected by `chart-style.svelte`, so layerchart series with `color: 'var(--color-{key})'` resolve through the chart context. Reusable dashboard charts (`UploadsChart`, `EngagementChart`, `FormatDistributionChart`) live in `$lib/components/dashboard/` and are shared between user and lab dashboards. When series labels come from props, wrap the `ChartConfig` in `$derived.by(() => ({...}))` — a static literal triggers Svelte's `state_referenced_locally` warning.
...
**Async side-effects on writes**: document creates/updates/deletes also (a) update Redis enrichment status when appropriate, (b) invalidate dashboard stats caches — both `dashboardStatsKey(userID)` and, when `LabID != nil && Visibility == DocVisibilityLab`, `labDashboardStatsKey(labID)` via the `invalidateLabDashboard()` helper in `service/document.go`, (c) trigger `EnrichDocument` gRPC where relevant. These are best-effort — log-and-continue on failure; don't fail the request. Lab membership changes (join/leave/kick in `service/lab.go`) also drop the lab cache because `member_count` lives in the lab dashboard payload.

**Dashboard stats parity**: user and lab dashboards share repo helpers, DTO row types (`DayCount`, `FormatBucket`, `StatusBreakdown`), and a `zeroFillDays(rows, days)` helper in `service/stats.go` — Postgres only returns days that had activity, but the API always returns a dense 30-entry series so the chart x-axis is continuous. The lab endpoint gates access at the **service** layer via `labRepo.FindMember` returning `app_error.ErrNotMember` (mapped to `service.get_lab_dashboard_stats.forbidden`), not just at the route. Both caches use `dashboardStatsTTL = 1h`.

**Time-window SQL with bound int**: pgx can't text-encode an `int` for the `(? || ' days')::interval` pattern (fails with `unable to encode N into text format for text (OID 25)`). Use `NOW() - make_interval(days => ?)` instead — `make_interval` accepts an integer parameter natively. Same trick for any other `make_interval` unit (hours, weeks, etc.) when the window length is user-tunable.
